			...
			<div class="d-flex">
				...
				<div style="flex: 3" class="mt-2 ml-3">
					...
					<div class="progress my-1 mb-2">
			                      <div class="progress-bar bg-success" role="progressbar" style="width: {{$member->own_progress}}%;" aria-valuenow="{{$member->own_progress}}" aria-valuemin="0" aria-valuemax="100">{{$member->own_progress}}%</div>
			                    </div>
				</div>
			</div>
		</div>
	...
	<div class="row mb-3">
		...
		<div class="col-sm-12">
			...
			<a href="#newTask" data-toggle="modal" class="btn btn-primary btn-sm float-right mt-3"><i class="far fa-plus-square"></i> New Task</a>
				...
				<h4 class="py-3 font-weight-bold float-left">{{ auth()->id() == $member->user->id ? 'My '  : $member->user->name . ' \'s'  }} Team</h4>
